var gatling = (() => {
  ...
  __export(stdin_exports, {
    ...
    deliveroo: () => deliveroo_gatling_default
  });

  // src/computerdatabase.gatling.ts
  var import_core = __toESM(require_target(), 1);
  var import_http = __toESM(require_target2(), 1);
  var computerdatabase_gatling_default = (0, import_core.simulation)((setUp) => {
    const feeder = (0, import_core.csv)("search.csv").random();
    const search = (0, import_core.exec)(
      (0, import_http.http)("Home").get("/"),
      (0, import_core.pause)(1),
      (0, import_core.feed)(feeder),
      (0, import_http.http)("Search").get("/computers?f=#{searchCriterion}").check((0, import_core.css)("a:contains('#{searchComputerName}')", "href").saveAs("computerUrl")),
      (0, import_core.pause)(1),
      (0, import_http.http)("Select").get("#{computerUrl}").check((0, import_http.status)().is(200)),
      (0, import_core.pause)(1)
    );
    const browse = (
      // Note how we force the counter name, so we can reuse it
      (0, import_core.repeat)(4, "i").on((0, import_http.http)("Page #{i}").get("/computers?p=#{i}"), (0, import_core.pause)(1))
    );
    const edit = (
      // let's try at max 2 times
      (0, import_core.tryMax)(2).on(
        (0, import_http.http)("Form").get("/computers/new"),
        (0, import_core.pause)(1),
        (0, import_http.http)("Post").post("/computers").formParam("name", "Beautiful Computer").formParam("introduced", "2012-05-30").formParam("discontinued", "").formParam("company", "37").check(
          (0, import_http.status)().is(
    ...
    const httpProtocol = import_http.http.baseUrl("https://computer-database.gatling.io").acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8").acceptLanguageHeader("en-US,en;q=0.5").acceptEncodingHeader("gzip, deflate").userAgentHeader(
      "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:109.0) Gecko/20100101 Firefox/119.0"
    );
    const users = (0, import_core.scenario)("Users").exec(search, browse);
    const admins = (0, import_core.scenario)("Admins").exec(search, browse, edit);
    setUp(
      users.injectOpen((0, import_core.rampUsers)(1e3).during(10))
    ).protocols(httpProtocol);
  });

  // src/deliveroo.gatling.ts
  var import_http2 = __toESM(require_target2(), 1);
  var import_core2 = __toESM(require_target(), 1);
  var deliveroo_gatling_default = (0, import_core2.simulation)((setUp) => {
    const httpProtocol = import_http2.http.baseUrl("http://localhost:3000").acceptHeader("application/json").contentTypeHeader("application/json");
    const scenarioGet = (0, import_core2.scenario)("get users").exec((0, import_http2.http)("Request get users").get("/user"));
    setUp(
      scenarioGet.injectOpen((0, import_core2.constantUsersPerSec)(20).during(15))
    ).protocols(httpProtocol);
  });
